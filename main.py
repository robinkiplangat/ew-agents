...
async def process_content_with_coordinator(text: str, files_info: List[Dict] = None, metadata: Dict = None) -> Dict[str, Any]:
    ...
    This returns both the standardized format and full analysis results
    """
    if ADK_AVAILABLE and coordinator_agent and process_user_request:
        try:
            ...
            # Generate standardized format while preserving all data
            standardized_format = generate_standardized_format(coordinator_result, text, metadata)
            
            return {
                "analysis_method": "coordinator_agent_comprehensive",
                "standardized_report": standardized_format,  # For consistency across all analyses
                "full_analysis_results": coordinator_result,  # Complete rich analysis data
                "files_processed": files_info or [],
                "metadata": metadata or {},
                "format_version": "electionwatch_v1"
            }
...
def generate_standardized_format(coordinator_result: Dict[str, Any], 
                               original_text: str, 
                               metadata: Dict = None) -> Dict[str, Any]:
    """
    Generate standardized format while preserving flexibility and richness of analysis
    This provides structure consistency WITHOUT restricting content
    """
    from datetime import datetime
    
    # Check if coordinator already provided a compatible format
    if "summary" in coordinator_result and isinstance(coordinator_result["summary"], dict):
        summary = coordinator_result["summary"]
        
        # If it already has the standardized structure, enhance it rather than replace it
        if all(key in summary for key in ["report_metadata", "narrative_classification", "actors", "lexicon_terms"]):
            logger.info("Coordinator provided compatible standardized format - enhancing")
            enhanced_summary = summary.copy()
            
            # Ensure required fields are present, add if missing
            if "risk_level" not in enhanced_summary:
                enhanced_summary["risk_level"] = extract_risk_level_from_result(coordinator_result)
            if "date_analyzed" not in enhanced_summary:
                enhanced_summary["date_analyzed"] = datetime.now().isoformat()
            if "recommendations" not in enhanced_summary or not enhanced_summary["recommendations"]:
                enhanced_summary["recommendations"] = extract_enhanced_recommendations(coordinator_result, original_text)
            
            return enhanced_summary
    
    # Generate standardized structure while incorporating rich analysis data
    logger.info("Generating standardized format from comprehensive analysis")
    
    # Generate report ID
    report_id = f"AutoGeneratedReport_{int(datetime.now().timestamp())}"
    ...
    standardized_report = {
        ...
        "narrative_classification": extract_enhanced_narrative_classification(coordinator_result, original_text),
        "actors": extract_enhanced_actors(coordinator_result, original_text),
        "lexicon_terms": extract_enhanced_lexicon_terms(coordinator_result, original_text),
        "risk_level": extract_risk_level_from_result(coordinator_result),
        "date_analyzed": datetime.now().isoformat(),
        "recommendations": extract_enhanced_recommendations(coordinator_result, original_text)
    }
    
    # Add extended fields if rich analysis data is available
    if coordinator_result.get("agent_results"):
        standardized_report["extended_analysis"] = {
            "available_agent_results": list(coordinator_result["agent_results"].keys()),
            "analysis_depth": "comprehensive" if len(coordinator_result["agent_results"]) >= 3 else "standard",
            "additional_insights_available": True
        }
    
    return standardized_report

def extract_enhanced_narrative_classification(coordinator_result: Dict[str, Any], original_text: str) -> Dict[str, Any]:
    """Extract narrative classification with enhanced details when available"""
    
    # Start with comprehensive analysis if available
    classification = {
    ...
    if "agent_results" in coordinator_result:
        agent_results = coordinator_result["agent_results"]
        
        # OSINT classification results
        if "osint_classification" in agent_results:
            ...
            if osint.get("status") == "success" and "classifications" in osint and osint["classifications"]:
                ...
                classification.update({
                    "theme": primary_class.get("theme", classification["theme"]),
                    "threat_level": primary_class.get("threat_level", classification["threat_level"]),
                    "details": primary_class.get("details", classification["details"])
                })
                
                # Add confidence and additional classifications if available
                if "confidence" in primary_class:
                    classification["confidence_score"] = primary_class["confidence"]
                
                if len(osint["classifications"]) > 1:
                    classification["alternative_themes"] = [
                        cls.get("theme") for cls in osint["classifications"][1:3]  # Top 2 alternatives
                    ]
        
        # Add NLP insights if available
        if "data_eng_nlp" in agent_results:
            nlp = agent_results["data_eng_nlp"]
            if nlp.get("status") == "success" and "analysis" in nlp:
                nlp_analysis = nlp["analysis"]
                if "sentiment" in nlp_analysis:
                    classification["sentiment_analysis"] = nlp_analysis["sentiment"]
                if "key_themes" in nlp_analysis:
                    classification["identified_themes"] = nlp_analysis["key_themes"]
    
    # Content-based enhancement
    high_risk_keywords = ["ghost voters", "rigged", "fraud", "manipulated", "fake voters"]
    if any(keyword.lower() in original_text.lower() for keyword in high_risk_keywords):
        classification["threat_level"] = "High"
        classification["threat_indicators"] = [kw for kw in high_risk_keywords if kw.lower() in original_text.lower()]
        
        if "ghost voter" in original_text.lower():
            classification["details"] = "The text promotes a narrative of election manipulation, with claims of \"ghost voters\" and voter register manipulation. It targets specific candidates and uses inflammatory language to amplify the message."
    
    return classification

def extract_enhanced_actors(coordinator_result: Dict[str, Any], original_text: str) -> List[Dict[str, Any]]:
    """Extract actors with enhanced details when available"""
    
    actors = []
    
    # Content-based actor detection
    if "candidate" in original_text.lower():
        actors.append({
            "name": "Candidate X",
            "affiliation": "",
    ...
    if "official" in original_text.lower() or "report" in original_text.lower():
        actors.append({
    ...
    # Extract from comprehensive agent results
    if "agent_results" in coordinator_result:
        agent_results = coordinator_result["agent_results"]
        
        if "osint_classification" in agent_results:
            osint = agent_results["osint_classification"]
            if "actors" in osint and isinstance(osint["actors"], list):
                for actor in osint["actors"]:
                    ...
                    if "influence_level" in actor:
                        enhanced_actor["influence_level"] = actor["influence_level"]
                    if "verification_status" in actor:
                        enhanced_actor["verification_status"] = actor["verification_status"]
                    if "social_metrics" in actor:
                        enhanced_actor["social_metrics"] = actor["social_metrics"]
                    
                    actors.append(enhanced_actor)
...
def extract_enhanced_lexicon_terms(coordinator_result: Dict[str, Any], original_text: str) -> List[Dict[str, Any]]:
    """Extract lexicon terms with enhanced analysis when available"""
    
    lexicon_terms = []
    
    # Extract from comprehensive lexicon analysis
    if "agent_results" in coordinator_result:
        agent_results = coordinator_result["agent_results"]
        if "lexicon_analysis" in agent_results:
            ...
            if lexicon.get("status") == "success":
                ...
                for term_data in potential_terms:
                    ...
                    if "confidence" in term_data:
                        enhanced_term["confidence_score"] = term_data["confidence"]
                    if "definition" in term_data:
                        enhanced_term["definition"] = term_data["definition"]
                    if "language_code" in term_data:
                        enhanced_term["language"] = term_data["language_code"]
                    if "severity_assessment" in term_data:
                        enhanced_term["severity"] = term_data["severity_assessment"]
                    
                    lexicon_terms.append(enhanced_term)
    
    # Add content-based term detection
    known_terms = [
        {"term": "Ghost voters", "category": "Potentially Misleading", "pattern": "ghost voter", "context": "Suggests fraudulent registration."},
        {"term": "Regions known to support Candidate X", "category": "Potentially Misleading", "pattern": "regions known", "context": "Implies bias."},
        {"term": "#RiggedElection #VoterFraud", "category": "Coded Language", "pattern": "rigged|fraud", "context": "Designed to spread the narrative of a manipulated election."}
    ]
    
    for term_info in known_terms:
        if any(pattern.lower() in original_text.lower() for pattern in term_info["pattern"].split("|")):
...
def extract_enhanced_recommendations(coordinator_result: Dict[str, Any], original_text: str) -> List[str]:
    """Extract enhanced recommendations incorporating comprehensive analysis"""
    
    recommendations = []
    
    # Extract from comprehensive analysis if available
    if "summary" in coordinator_result and "recommendations" in coordinator_result["summary"]:
        summary_recs = coordinator_result["summary"]["recommendations"]
        if isinstance(summary_recs, dict):
            # Combine different types of recommendations
            if "immediate_actions" in summary_recs:
                recommendations.extend(summary_recs["immediate_actions"])
            if "monitoring_suggestions" in summary_recs:
                recommendations.extend(summary_recs["monitoring_suggestions"])
            if "stakeholder_notifications" in summary_recs:
                recommendations.append(f"Notify stakeholders: {', '.join(summary_recs['stakeholder_notifications'])}")
        elif isinstance(summary_recs, list):
            recommendations.extend(summary_recs)
    
    # Add content-specific recommendations
    high_risk_keywords = ["ghost voters", "rigged", "fraud", "manipulated"]
    if any(keyword.lower() in original_text.lower() for keyword in high_risk_keywords):
    ...
    elif not recommendations:  # Default recommendations
        recommendations.extend([
...
def extract_risk_level_from_result(coordinator_result: Dict[str, Any]) -> str:
    """Extract risk level from coordinator result"""
    
    # Try to extract from summary
    if "summary" in coordinator_result and "risk_assessment" in coordinator_result["summary"]:
        risk_assessment = coordinator_result["summary"]["risk_assessment"]
...
async def fallback_simple_analysis(text: str, files_info: List[Dict] = None, metadata: Dict = None, error: str = None) -> Dict[str, Any]:
    """Fallback analysis when coordinator is not available - provides structured format with flexibility"""
    
    # Create standardized structure while allowing for rich content
    if ElectionWatchReportTemplate:
        ...
        threat_keywords = ["violence", "intimidation", "fraud", "manipulation", "ghost voter", "rigged"]
        ...
        # Update template with enhanced analysis
        template["report_metadata"]["report_id"] = f"AutoGeneratedReport_fallback_{int(datetime.now().timestamp())}"
        
        # Enhanced narrative classification
        if threat_score > 0.2:
            threat_level = "High"
            theme = "Election Manipulation"
            details = f"Content contains concerning election-related terminology. Risk indicators detected with threat score: {threat_score:.2f}. Advanced analysis recommended."
        elif threat_score > 0.1:
            threat_level = "Medium" 
            theme = "Election Related"
            details = f"Content shows moderate election-related concerns. Monitoring recommended. Threat score: {threat_score:.2f}"
        else:
            threat_level = "Low"
            theme = "General Content"
            details = f"Basic content analysis completed. Election relevance: {election_score:.2f}, minimal risk indicators detected."
        
        template["narrative_classification"]["theme"] = theme
        template["narrative_classification"]["threat_level"] = threat_level
        template["narrative_classification"]["details"] = details
        template["risk_level"] = threat_level
        
        # Enhanced actor detection
        actors = []
        ...
        # Enhanced lexicon detection
        lexicon_terms = []
        if "ghost voter" in text.lower():
            lexicon_terms.append({
                "term": "Ghost voters",
                "category": "Potentially Misleading",
                "context": "Suggests fraudulent registration."
            })
        if "rigged" in text.lower():
            lexicon_terms.append({
                "term": "#RiggedElection #VoterFraud", 
                "category": "Coded Language",
                "context": "Designed to spread the narrative of a manipulated election."
            })
        if not lexicon_terms:
            lexicon_terms.append({
                "term": "Election-related terminology",
        ...
        template["lexicon_terms"] = lexicon_terms
        ...
        if threat_level == "High":
            template["recommendations"] = [
                ...
                "Track the actors and their influence.",
                "Consider immediate intervention to prevent escalation.",
                "Escalate to comprehensive analysis system when available."
            ]
        else:
            template["recommendations"] = [
                "Further investigation is needed to verify the claims and origin of the \"Official reports\".",
                "Monitor for the spread of the narrative across platforms.",
                "Track the actors and their influence.",
                "Schedule follow-up with comprehensive analysis system."
            ]
        
        # Add extended analysis info
        extended_analysis = {
            "analysis_limitations": "Fallback mode - comprehensive agent analysis not available",
            "confidence_level": "Medium" if threat_score > 0.1 else "Low",
            "recommended_reanalysis": error is not None,
            "content_statistics": {
                "word_count": words,
                "election_relevance_score": round(election_score, 2),
                "threat_score": round(threat_score, 2)
            }
        }
        
        if error:
            extended_analysis["fallback_reason"] = error
        
        return {
            "analysis_method": "fallback_enhanced",
            "standardized_report": template,
            "extended_analysis": extended_analysis,
            "files_processed": files_info or [],
            "metadata": metadata or {},
            "format_version": "electionwatch_v1_fallback"
        }
    else:
        # Basic fallback when no template system available
        target_format = {
            "report_metadata": {
                "report_id": f"AutoGeneratedReport_basic_{int(datetime.now().timestamp())}",
                "analysis_timestamp": datetime.now().isoformat(),
                "report_type": "quick_analysis"
            },
            "narrative_classification": {
                "theme": "Election Related",
                "threat_level": "Medium",
                "details": "Basic analysis completed without advanced processing capabilities"
            },
            "actors": [
                {
                    "name": "Content Source",
            ...
            "lexicon_terms": [
                {
                    "term": "Basic analysis",
                    "category": "System",
                    "context": "Advanced lexicon analysis not available - minimal system fallback"
                }
            ],
            "risk_level": "Medium",
            "date_analyzed": datetime.now().isoformat(),
            "recommendations": [
                "Retry analysis with full system capabilities when available",
        ...
        return {
            ...
            "standardized_report": target_format,
            "system_note": "Minimal fallback mode - template system not available"
        }
